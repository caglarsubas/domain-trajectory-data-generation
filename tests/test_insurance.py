...
def _claim_share(bundle):
    primaries = [item for item in bundle.trajectories if item.parent_trajectory_id is None]
    return sum("claim.notified" in _types(bundle, item) for item in primaries) / len(primaries)


def test_warm_corpus_weights_a_named_claim_unless_the_domain_excludes_it():
    scope = ["quoting", "underwriting", "policy_administration", "billing", "claims"]
    common = {"start_mode": "warm", "sub_domains": scope, "seed": "events", "target_trajectory_count": 40, "min_events": 6, "max_events": 16, "event_budget": None}
    named = _bundle(corpus_text="Public notes: claim.notified after the policy is in force. Customers pay USD on mobile for motor cover.", **common)
    plain = _bundle(corpus_text="Customers pay USD on mobile for motor cover.", **common)
    assert _claim_share(named) > _claim_share(plain) + 0.1
    ...
    assert any(obj.subtype == "motor" for obj in named.objects if obj.object_type == "product_offering")
    assert insurance_hard_checks(named) == []
    outside = _bundle(
        start_mode="warm",
        sub_domains=["quoting", "underwriting", "policy_administration", "billing"],
    ...
    assert all(event.event_type != "claim.notified" for event in outside.events)
    cold = _bundle(
        start_mode="cold",
        sub_domains=scope,
...
def test_settlement_before_assessment_is_rejected():
    bundle = _bundle(sub_domains=["claims", "policy_administration"], target_trajectory_count=12, min_events=4, max_events=20, event_budget=None, seed="order")
    events = {event.event_id: event for event in bundle.events}
