...
`banking-semi-markov-v1` fills these records for a banking run. `insurance-semi-markov-v1` fills them for a retail insurance run: quote, underwriting, policy, premium, and claim. Domain order is fixed, dwell times vary, and a branch is a simulated alternative rather than a causal counterfactual. The studio stores at most 64 primary trajectories for a run, and fewer when the event budget is exhausted. Warm-start text contributes only matched terms such as currency, channel, product, and named events from that sector. Cold start ignores the corpus.
