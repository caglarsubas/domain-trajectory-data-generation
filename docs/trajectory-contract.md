...
Each pack also carries `operations`, a map from event to (service domain, action) after its industry's API model (BIAN, insurance capability domains, TM Forum Open APIs, IATA NDC and ONE Order), which names episode operations, and `agent`, the episode agent's system text and task per language. `sectors.gates.run_gates` checks a pack against the gates banking passes; only packs that pass are registered and served by `GET /sectors`, which adds each pack's `default_sub_domains`. A jurisdiction profile's `rules` give a sector its own rules in place of KYC in the system segment and the judge brief.
