...
- `airline-semi-markov-v1` (pack `airline-pack-1`) covers booking with payment captured, failed, or expired before ticketing; seats, bags, changes that may be declined, and voluntary cancellation with a refund; check-in, a bag dropped before boarding, and boarding, a no-show, or denied boarding; delays, cancellations, and rebooking or refund after a disruption; arrival on time or, after a delay, more than three hours late; compensation claims after a late arrival, a cancellation, or denied boarding; delivered, delayed, and returned bags; loyalty enrolment and miles; and complaints resolved or escalated.
- `hotel-semi-markov-v1` (pack `hotel-pack-1`) covers reservations guaranteed and confirmed, declined, or lapsed; changes and cancellations before arrival, with a refund or a late-cancellation fee; room assignment, check-in, no-shows, and guests walked from an oversold house with compensation; service requests, room issues, and folio charges during the stay; folio settlement or dispute after check-out; loyalty points and reviews once the stay has ended; and complaints resolved or escalated.

Each pack also carries `operations`, a map from event to (service domain, action) after its industry's API model (BIAN, insurance capability domains, TM Forum Open APIs, IATA NDC and ONE Order, HTNG and OpenTravel), which names episode operations, and `agent`, the episode agent's system text and task per language. `sectors.gates.run_gates` checks a pack against the gates banking passes; only packs that pass are registered and served by `GET /sectors`, which adds each pack's `default_sub_domains`. A jurisdiction profile's `rules` give a sector its own rules in place of KYC in the system segment and the judge brief.
