...
Progress, 25 September 2026: Slice 3 ships in three pull requests. The first puts generation behind a job queue on the application database: a `jobs` table, a worker that runs as its own Compose service, a background thread, or inline on SQLite, progress reports, cancellation before and during a run, requeueing of jobs whose worker stopped, and a run list that no longer loads any bundle. On Postgres, four concurrent workers ran 40 jobs with each run exactly once. The second pull request removes the cap: batched, resumable generation, bundles stored outside the database, journeys served a page at a time, and oversampling to a target of accepted samples. The third moves judging and deep search into jobs and adds demo quotas and live key validation.
